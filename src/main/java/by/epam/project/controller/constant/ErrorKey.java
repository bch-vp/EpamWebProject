package by.epam.project.controller.constant;

public class ErrorKey {
    public static final String ERROR = "error";

    public static final String LOGIN_NOT_FOUND = "login_not_found";
    public static final String LOGIN_NOT_UNIQUE = "login_not_unique";

    public static final String TELEPHONE_NUMBER_NOT_UNIQUE = "telephone_number_not_unique";

    public static final String EMAIL_NOT_UNIQUE = "email_not_unique";

    public static final String DATABASE_CONNECTION_NOT_RECEIVED = "database_connection_not_received";

    private ErrorKey(){}
}
